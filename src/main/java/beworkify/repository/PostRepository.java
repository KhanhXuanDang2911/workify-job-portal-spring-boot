...
public interface PostRepository extends JpaRepository<Post, Long> {
	@Query("select p from Post p join p.category c" + " where (:categoryId is null or c.id = :categoryId)"
	...
	@Query("select p from Post p join p.category c" + " where (:categoryId is null or c.id = :categoryId)"
			+ " and (lower(p.title) like concat('%', :keyword, '%')"
			+ " OR lower(p.content) like concat('%', :keyword, '%')"
			+ " OR lower(p.author.fullName) like concat('%', :keyword, '%'))" + " and p.status = :publicStatus")
	...
	@Query(value = """
			...
			LIMIT :limit
			""", nativeQuery = true)
	List<Post> findRelatedByFullTextSearch(@Param("excludeId") Long excludeId, @Param("searchText") String searchText,
	...
	@Query(value = """
			...
			LIMIT :limit
			""", nativeQuery = true)
	List<Post> findRelatedBySimilarity(@Param("excludeId") Long excludeId, @Param("title") String title,
			@Param("contentText") String contentText, @Param("limit") int limit);
	...
	@Query("""
			...
			""")
	List<Post> findRelatedByCategory(@Param("status") StatusPost status, @Param("categoryId") Long categoryId,
	...
	@Query("""
			...
			ORDER BY p.updatedAt DESC
			""")
	List<Post> findLatestPosts(@Param("status") StatusPost status, @Param("excludeId") Long excludeId,
			Pageable pageable);
