...
public interface JobMapper {
	...
	default void mapIndustries(@MappingTarget JobResponse jobResponse, Job job) {
		if (job.getJobIndustries() != null && !job.getJobIndustries().isEmpty()) {
			Set<IndustryResponse> industries = job.getJobIndustries().stream()
					.map(jobIndustry -> jobIndustry != null ? jobIndustry.getIndustry() : null).filter(Objects::nonNull)
					.map(industry -> IndustryResponse.builder().id(industry.getId()).createdAt(industry.getCreatedAt())
							.updatedAt(industry.getUpdatedAt()).name(industry.getName()).engName(industry.getEngName())
							.description(industry.getDescription()).build())
	...
	default void mapAuthor(@MappingTarget JobResponse jobResponse, Job job) {
		jobResponse.setAuthor(EmployerSummaryResponse.builder().email(job.getAuthor().getEmail())
				.backgroundUrl(job.getAuthor().getBackgroundUrl()).avatarUrl(job.getAuthor().getAvatarUrl())
				.companyName(job.getAuthor().getCompanyName()).employerSlug(job.getAuthor().getEmployerSlug())
				.id(job.getAuthor().getId()).createdAt(job.getAuthor().getCreatedAt())
				.updatedAt(job.getAuthor().getUpdatedAt()).build());
	}
	...
	default void mapJobBenefits(@MappingTarget Job job, JobRequest request) {
		if (request.getJobBenefits() != null && !request.getJobBenefits().isEmpty()) {
			job.setJobBenefits(request.getJobBenefits().stream()
					.map(benefitRequest -> JobBenefit.builder().type(BenefitType.fromValue(benefitRequest.getType()))
							.description(benefitRequest.getDescription()).build())
