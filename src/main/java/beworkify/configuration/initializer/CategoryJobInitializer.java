...
public class CategoryJobInitializer implements CommandLineRunner {
	...
	public void run(String... args) {
		try {
			...
			List<Entry> entries = Arrays.asList(new Entry("Bộ phận hỗ trợ", "Administrative / Support Department"),
					new Entry("Hỗ trợ sản xuất", "Production Support"),
					new Entry("Xây dựng / Bất động sản", "Construction / Real Estate"),
					new Entry("Truyền thông", "Media / Communication"), new Entry("Dịch vụ", "Services"),
					...
					new Entry("Kỹ thuật", "Engineering"), new Entry("Sản xuất", "Manufacturing"),
					new Entry("Theo đối tượng", "By Target Group"), new Entry("Khác", "Others"));

			for (Entry e : entries) {
				if (!categoryJobRepository.existsByName(e.vi)) {
					categoryJobRepository
							.save(CategoryJob.builder().name(e.vi).engName(e.en).description(null).build());
				}
			}
