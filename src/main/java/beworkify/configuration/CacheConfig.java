...
public class CacheConfig {
        ...
        public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory,
                ...
                cacheConfigs.put("roles",
                                cacheConfiguration(redisSerializer).entryTtl(Duration.ofDays(7)));
                cacheConfigs.put("provinces",
                                cacheConfiguration(redisSerializer).entryTtl(Duration.ofDays(2)));
                cacheConfigs.put("districts",
                                cacheConfiguration(redisSerializer).entryTtl(Duration.ofDays(2)));
                cacheConfigs.put("categories_job",
                                cacheConfiguration(redisSerializer).entryTtl(Duration.ofDays(2)));
                cacheConfigs.put("categories_job_count",
                                cacheConfiguration(redisSerializer)
                                                .entryTtl(Duration.ofSeconds(90)));
                cacheConfigs.put("categories_post",
                                cacheConfiguration(redisSerializer).entryTtl(Duration.ofDays(2)));
