  ...
  public boolean isValid(JobRequest job, ConstraintValidatorContext context) {
    ...
    switch (type) {
      case RANGE -> {
        if (minSalary == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.salary.range.minRequired}")
        ...
        if (maxSalary == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.salary.range.maxRequired}")
        ...
        if (unit == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.salary.range.unitRequired}")
              .addPropertyNode("salaryUnit")
              .addConstraintViolation();
          valid = false;
        }
      }
      case GREATER_THAN -> {
        if (minSalary == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.salary.greater.minRequired}")
              .addPropertyNode("minSalary")
              .addConstraintViolation();
          valid = false;
        }
        if (unit == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.salary.greater.unitRequired}")
              .addPropertyNode("salaryUnit")
              .addConstraintViolation();
          valid = false;
        }
