  ...
  public boolean isValid(JobRequest job, ConstraintValidatorContext context) {
    ...
    switch (type) {
      ...
      case ABOVE -> {
        if (min == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.age.above.minRequired}")
      ...
      case BELOW -> {
        if (max == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.age.below.maxRequired}")
      ...
      case INPUT -> {
        if (min == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.age.input.minRequired}")
              .addPropertyNode("minAge")
              .addConstraintViolation();
          valid = false;
        }
        if (max == null) {
          context
              .buildConstraintViolationWithTemplate("{validation.age.input.maxRequired}")
              .addPropertyNode("maxAge")
              .addConstraintViolation();
          valid = false;
        }
        if (min != null && max != null && min > max) {
          context
              .buildConstraintViolationWithTemplate("{validation.age.input.rangeInvalid}")
              .addPropertyNode("maxAge")
              .addConstraintViolation();
          valid = false;
        }
