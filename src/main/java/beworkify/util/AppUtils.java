...
public class AppUtils {
  ...
  public static Pageable generatePageableWithSort(
      List<String> sorts, List<String> whileListFieldSorts, int pageNumber, int pageSize) {
    Pageable pageable;
      ...
      orders =
          sorts.stream()
              ...
              .map(
                  s -> {
                    String[] sortPart = s.split(":");
                    if (sortPart[1].equalsIgnoreCase("asc")) return Sort.Order.asc(sortPart[0]);
                    else return Sort.Order.desc(sortPart[0]);
                  })
              .toList();
    }
    if (orders.isEmpty()) pageable = PageRequest.of(pageNumber - 1, pageSize);
    else pageable = PageRequest.of(pageNumber - 1, pageSize, Sort.by(orders));
    return pageable;
  }
  ...
  public static Long getEmployerIdFromSecurityContext() {
    ...
    if (!(principal instanceof Employer employer)) {
      return null;
    }
