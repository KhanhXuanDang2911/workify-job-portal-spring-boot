...
public class UserServiceImpl implements UserService {
	...
	public PageResponse<List<UserResponse>> getUsersWithPaginationAndKeywordAndSorts(int pageNumber, int pageSize,
		...
		log.info("Fetched {} users (page {}/{})", userPage.getNumberOfElements(), pageNumber, userPage.getTotalPages());

		return PageResponse.<List<UserResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
				.totalPages(userPage.getTotalPages()).numberOfElements(userPage.getNumberOfElements())
				.items(userResponses).build();
	}
	...
	public UserResponse signUp(UserRequest request, boolean isMobile)
		...
		User user = User.builder().role(role).email(request.getEmail()).fullName(request.getFullName())
				.password(passwordEncoder.encode(request.getPassword())).status(StatusUser.PENDING).noPassword(false)
				.build();
