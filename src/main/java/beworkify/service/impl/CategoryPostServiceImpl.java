...
public class CategoryPostServiceImpl implements CategoryPostService {
	...
	@Override
	@Transactional
	@Caching(evict = {@CacheEvict(value = "categories_post", key = "'all'")}, put = {
	...
	public CategoryPostResponse update(Long id, CategoryPostRequest request) {
		CategoryPost entity = repository.findById(id).orElseThrow(() -> {
			String message = messageSource.getMessage("categoryPost.notFound", null, LocaleContextHolder.getLocale());
	...
	public PageResponse<List<CategoryPostResponse>> getAllWithPaginationAndSort(int pageNumber, int pageSize,
		...
		Pageable pageable = AppUtils.generatePageableWithSort(sorts, List.of("title", "createdAt", "updatedAt"),
				pageNumber, pageSize);
		...
		return PageResponse.<List<CategoryPostResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
