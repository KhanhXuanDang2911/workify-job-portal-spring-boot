...
/**
 * Implementation of the NotificationService interface. Handles business logic for notifications,
 * including creation, retrieval, and WebSocket broadcasting.
 *
 * @author KhanhDX
 * @since 1.0.0
 */
...
public class NotificationServiceImpl implements NotificationService {
  ...
  public void notifyEmployer(
    ...
    messagingTemplate.convertAndSendToUser(
        "EMPLOYER:" + employer.getEmail(), "/queue/notifications", dto);
  }
                ...
                () -> {
                  String message =
                      messageSource.getMessage(
                          "notification.not.found", null, LocaleContextHolder.getLocale());
  ...
  public void markAllAsRead() {
    ...
    if (AppUtils.hasRole(authentication, "EMPLOYER")) {
      ...
      employer.setId(employerId);
      repository
          .findByRecipientEmployerOrderByCreatedAtDesc(employer, Pageable.unpaged())
          .forEach(
              n -> {
                if (!n.getReadFlag()) {
                  n.setReadFlag(true);
      ...
      user.setId(userId);
      repository
          .findByRecipientUserOrderByCreatedAtDesc(user, Pageable.unpaged())
          .forEach(
              n -> {
                if (!n.getReadFlag()) {
                  n.setReadFlag(true);
