...
public class EmployerServiceImpl implements EmployerService {
  ...
  @Override
  public PageResponse<List<EmployerResponse>> getEmployersWithPaginationAndKeywordAndSorts(
      int pageNumber,
      int pageSize,
      List<String> sorts,
      String keyword,
      LevelCompanySize companySize,
      Long provinceId,
      boolean isAdmin) {
  ...
  @PostAuthorize(
      "hasRole('ADMIN') or returnObject.status == T(beworkify.enumeration.StatusUser).ACTIVE")
  @Override
  ...
  @Override
  @CachePut(value = "employers", key = "#result.id", condition = "#result != null")
  public EmployerResponse createEmployer(
      EmployerRequest request, MultipartFile avatar, MultipartFile background) {
    existsByEmail(request.getEmail());
    Province province = provinceService.findProvinceById(request.getProvinceId());
    District district = districtService.findDistrictById(request.getDistrictId());
  ...
  public EmployerResponse updateEmployer(
      Long id, EmployerRequest request, MultipartFile avatar, MultipartFile background) {
    Employer employer = findEmployerById(id);
    if (request.getEmail() != null
        && employerRepository.existsByEmailAndIdNot(request.getEmail(), id)) {
      String message =
          messageSource.getMessage(
              "employer.email.exists",
        ...
        employerRepository
            ...
            .orElseThrow(
                () -> {
                  String message =
                      messageSource.getMessage(
                          "employer.not.found", new Object[] {id}, LocaleContextHolder.getLocale());
    ...
    return employerRepository
        ...
        .orElseThrow(
            () -> {
              ...
              String message =
                  messageSource.getMessage(
                      "employer.not.found", null, LocaleContextHolder.getLocale());
              return new ResourceNotFoundException(message);
            });
  }
  ...
  private void existsByEmail(String email) {
    if (employerRepository.existsByEmail(email)) {
      String message =
          messageSource.getMessage(
              "employer.email.exists", new Object[] {email}, LocaleContextHolder.getLocale());
      throw new ResourceConflictException(message);
    }
  }
  ...
  public void forgotPassword(ForgotPasswordRequest request, boolean isMobile)
    ...
    if (isMobile) {
      mailService.sendResetLink(
          employer, true, redisOTPCodeService.generateAndSaveOTPCode(request.getEmail(), 60));
    } else {
