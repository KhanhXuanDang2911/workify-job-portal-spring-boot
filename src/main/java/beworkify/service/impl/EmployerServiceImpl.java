...
public class EmployerServiceImpl implements EmployerService {
	...
	public PageResponse<List<EmployerResponse>> getEmployersWithPaginationAndKeywordAndSorts(int pageNumber,
			int pageSize, List<String> sorts, String keyword, LevelCompanySize companySize, Long provinceId,
			boolean isAdmin) {
		...
		Map<Long, Long> hiringCountMap = employerIds.isEmpty()
				? Map.of()
		...
		return PageResponse.<List<EmployerResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
	...
	public EmployerResponse signUpEmployer(EmployerRequest request, boolean isMobile)
		...
		Employer employer = Employer.builder().email(request.getEmail()).phoneNumber(request.getPhoneNumber())
				.password(passwordEncoder.encode(request.getPassword())).companyName(request.getCompanyName())
				.companySize(LevelCompanySize.fromLabel(request.getCompanySize()))
				.contactPerson(request.getContactPerson()).detailAddress(request.getDetailAddress())
				.status(StatusUser.PENDING).build();
	...
	private Employer findEmployerById(Long id) {
		return employerRepository.findById(id).orElseThrow(() -> {
			...
			String message = messageSource.getMessage("employer.not.found", null, LocaleContextHolder.getLocale());
			return new ResourceNotFoundException(message);
		});
	}
