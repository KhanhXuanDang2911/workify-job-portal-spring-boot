...
public class PostServiceImpl implements PostService {
	...
	public PostResponse create(PostRequest request, MultipartFile thumbnail) throws Exception {
		...
		author = userRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException(
				messageSource.getMessage("user.not.found.by.id", new Object[]{id}, LocaleContextHolder.getLocale())));
		...
		response.setAuthor(UserSummaryResponse.builder().id(entity.getAuthor().getId())
				.avatarUrl(entity.getAuthor().getAvatarUrl()).email(entity.getAuthor().getEmail())
				.fullName(entity.getAuthor().getFullName()).role(entity.getAuthor().getRole().getRole().getName())
				.build());
	...
	public PostResponse update(Long id, PostRequest request, MultipartFile thumbnail) throws Exception {
		...
		Post entity = repository.findById(id).orElseThrow(() -> new ResourceNotFoundException(
	...
	@Override
	@PostAuthorize("hasRole('ADMIN') " + "or returnObject.status == T(beworkify.enumeration.StatusPost).PUBLIC")
	...
	public PostResponse getById(Long id) {
		...
		PostResponse response = mapper.toDTO(entity);
		response.setAuthor(UserSummaryResponse.builder().id(entity.getAuthor().getId())
				.avatarUrl(entity.getAuthor().getAvatarUrl()).fullName(entity.getAuthor().getFullName())
				.email(entity.getAuthor().getEmail()).role(entity.getAuthor().getRole().getRole().getName()).build());
		return response;
	}
	...
	public PageResponse<List<PostResponse>> getAll(int pageNumber, int pageSize, List<String> sorts, String keyword,
		...
		List<PostResponse> items = page.getContent().stream().map(entity -> {
			...
			response.setAuthor(UserSummaryResponse.builder().id(entity.getAuthor().getId())
					.avatarUrl(entity.getAuthor().getAvatarUrl()).fullName(entity.getAuthor().getFullName())
					.role(entity.getAuthor().getRole().getRole().getName()).email(entity.getAuthor().getEmail())
					.build());
			return response;
		}).toList();
		return PageResponse.<List<PostResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
	...
	public List<PostResponse> getRelated(Long postId, int limit) {
		Post anchor = repository.findById(postId).orElseThrow(() -> new ResourceNotFoundException(
				messageSource.getMessage("error.resource.not.found", null, LocaleContextHolder.getLocale())));
		...
		String searchText = cleanSearchText(
				anchor.getTitle() + " " + (anchor.getContentText() != null ? anchor.getContentText() : ""));
		...
		if (related.size() < limit) {
			...
			try {
				var similarityResults = repository.findRelatedBySimilarity(anchor.getId(), anchor.getTitle(),
						anchor.getContentText() != null ? anchor.getContentText() : "", remaining);
		...
		if (related.size() < limit && anchor.getCategory() != null) {
			int remaining = limit - related.size();
			var categoryResults = repository.findRelatedByCategory(StatusPost.PUBLIC, anchor.getCategory().getId(),
					anchor.getId(), PageRequest.of(0, remaining));
			addUniqueResults(related, addedIds, categoryResults);
		}

		if (related.size() < limit) {
			int remaining = limit - related.size();
			var latestResults = repository.findLatestPosts(StatusPost.PUBLIC, anchor.getId(),
					PageRequest.of(0, remaining));
			addUniqueResults(related, addedIds, latestResults);
		}

		return related.stream().limit(limit).map(p -> {
			PostResponse response = mapper.toDTO(p);
			response.setAuthor(UserSummaryResponse.builder().id(p.getAuthor().getId())
					.avatarUrl(p.getAuthor().getAvatarUrl()).fullName(p.getAuthor().getFullName())
					.role(p.getAuthor().getRole().getRole().getName()).email(p.getAuthor().getEmail()).build());
			return response;
		}).toList();
	}
	...
	public List<PostResponse> getLatestPosts(int limit) {
		...
		return posts.stream().map(entity -> {
			PostResponse response = mapper.toDTO(entity);
			response.setAuthor(UserSummaryResponse.builder().id(entity.getAuthor().getId())
					.avatarUrl(entity.getAuthor().getAvatarUrl()).fullName(entity.getAuthor().getFullName())
					.role(entity.getAuthor().getRole().getRole().getName()).email(entity.getAuthor().getEmail())
