...
public class ProvinceServiceImpl implements ProvinceService {
	...
	@Override
	@Transactional
	@Caching(evict = {@CacheEvict(value = "provinces", key = "'all'")}, put = {
	...
	public ProvinceResponse update(Long id, ProvinceRequest request) {
		Province entity = repository.findById(id).orElseThrow(() -> {
	...
	@Override
	@Transactional
	@Caching(evict = {@CacheEvict(value = "provinces", key = "'all'"), @CacheEvict(value = "provinces", key = "#id"),
	...
	public PageResponse<List<ProvinceResponse>> getAllWithPaginationAndSort(int pageNumber, int pageSize,
		...
		return PageResponse.<List<ProvinceResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
	...
	public Province findProvinceById(Long id) {
		return repository.findById(id).orElseThrow(() -> {
			String message = messageSource.getMessage("province.not.found", null, LocaleContextHolder.getLocale());
			return new ResourceNotFoundException(message);
		});
	}
