...
public class JobServiceImpl implements JobService {
	...
	public JobResponse update(Long id, JobRequest request) {
		...
		if (request.getJobLocations() != null) {
			...
			jobLocations.clear();
			List<Location> newJobLocations = createJobLocationsFromRequest(request.getJobLocations(), entity);
		...
		if (request.getIndustryIds() != null) {
			...
			List<JobIndustry> newJobIndustries = createJobIndustriesFromRequest(request.getIndustryIds(), entity);
	...
	public PageResponse<List<JobResponse>> getMyJobs(int pageNumber, int pageSize, Long industryId, Long provinceId,
		...
		List<String> WHITE_LIST_SORTS = Arrays.asList("jobTitle", "createdAt", "updatedAt", "expirationDate", "status");
	...
	public PageResponse<List<JobResponse>> getAllJobs(int pageNumber, int pageSize, Long industryId, Long provinceId,
			List<String> sorts, String keyword) {
		List<String> WHITE_LIST_SORTS = Arrays.asList("jobTitle", "createdAt", "updatedAt", "expirationDate", "status");
		keyword = keyword == null ? "" : keyword.trim();
		Pageable pageable = AppUtils.generatePageableWithSort(sorts, WHITE_LIST_SORTS, pageNumber, pageSize);
		Page<Long> page = jobRepository.findIdsAllJobs(provinceId, industryId, keyword.toLowerCase(), pageable);
		List<Long> orderedIds = page.getContent();
		List<Job> fetchedJobs = jobRepository.fetchJobsByIds(orderedIds);
		Map<Long, Job> jobById = fetchedJobs.stream().collect(Collectors.toMap(Job::getId, j -> j));
		List<Job> jobsOrdered = orderedIds.stream().map(jobById::get).filter(Objects::nonNull).toList();
	...
	public List<PopularLocationResponse> getPopularLocations(Integer limit) {
		...
		return results.stream().map(r -> {
			...
			return PopularLocationResponse.builder().id(province.getId()).code(province.getCode())
					.name(province.getName()).engName(province.getEngName()).provinceSlug(province.getProvinceSlug())
	...
	public List<PopularIndustryResponse> getPopularIndustries(Integer limit) {
		...
		return results.stream().map(r -> {
			...
			Long jobCount = (Long) r[1];
			return PopularIndustryResponse.builder().id(industry.getId()).name(industry.getName())
					.engName(industry.getEngName()).description(industry.getDescription()).jobCount(jobCount).build();
		}).toList();
	}
	...
	public Job findJobById(Long id) {
		return jobRepository.findById(id).orElseThrow(() -> {
			String message = messageSource.getMessage("job.notFound", null, LocaleContextHolder.getLocale());
	...
	private Location createLocationFromRequest(LocationRequest request) {
		...
		return Location.builder().province(province).district(district).detailAddress(request.getDetailAddress())
				.build();
	}
	...
	private List<Location> createJobLocationsFromRequest(List<LocationRequest> requests, Job job) {
		...
		for (LocationRequest request : requests) {
			...
			Location location = Location.builder().province(province).district(district)
					.detailAddress(request.getDetailAddress()).job(job).build();
