...
public class IndustryServiceImpl implements IndustryService {
	...
	@Override
	@Transactional
	@Caching(evict = {@CacheEvict(value = "industries", key = "'all'")}, put = {
	...
	public IndustryResponse create(IndustryRequest request) {
		...
		if (repository.existsByEngName(request.getEngName())) {
			String message = messageSource.getMessage("industry.exists.engName", new Object[]{request.getEngName()},
					LocaleContextHolder.getLocale());
			throw new ResourceConflictException(message);
		}
	...
	public Industry findIndustryById(Long id) {
		return repository.findById(id).orElseThrow(() -> {
			String message = messageSource.getMessage("industry.notFound", null, LocaleContextHolder.getLocale());
	...
	public PageResponse<List<IndustryResponse>> getAllWithPaginationAndSort(int pageNumber, int pageSize,
		...
		return PageResponse.<List<IndustryResponse>>builder().pageNumber(pageNumber).pageSize(pageSize)
