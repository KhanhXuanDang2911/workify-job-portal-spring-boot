...
public class IndustryServiceImpl implements IndustryService {
  ...
  @Transactional
  @Caching(
      evict = {@CacheEvict(value = "industries", key = "'all'")},
      put = {@CachePut(value = "industries", key = "#result.id")})
  public IndustryResponse create(IndustryRequest request) {
    ...
    if (repository.existsByEngName(request.getEngName())) {
      String message =
          messageSource.getMessage(
              "industry.exists.engName",
              new Object[] {request.getEngName()},
              LocaleContextHolder.getLocale());
      throw new ResourceConflictException(message);
    }
  ...
  @Caching(
      evict = {@CacheEvict(value = "industries", key = "'all'")},
      put = {@CachePut(value = "industries", key = "#id")})
  public IndustryResponse update(Long id, IndustryRequest request) {
    ...
    if (request.getEngName() != null
        && repository.existsByEngNameAndIdNot(request.getEngName(), id)) {
      String message =
          messageSource.getMessage(
              "industry.exists.engName",
              new Object[] {request.getEngName()},
    ...
    return repository
        ...
        .orElseThrow(
            () -> {
              String message =
                  messageSource.getMessage(
                      "industry.notFound", null, LocaleContextHolder.getLocale());
  ...
  @Cacheable(
      value = "industries",
      key =
          "@keyGenerator.buildKeyWithPaginationSortsKeywordForIndustries(#pageNumber, #pageSize, #sorts, #keyword, T(java.util.List).of(\"name\", \"engName\", \"createdAt\", \"updatedAt\"), #categoryId)")
  public PageResponse<List<IndustryResponse>> getAllWithPaginationAndSort(
    ...
    org.springframework.data.domain.Pageable pageable =
        AppUtils.generatePageableWithSort(
            sorts, List.of("name", "engName", "createdAt", "updatedAt"), pageNumber, pageSize);
