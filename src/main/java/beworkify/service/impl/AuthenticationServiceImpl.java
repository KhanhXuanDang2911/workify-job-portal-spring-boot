...
public class AuthenticationServiceImpl implements AuthenticationService {
	...
	public TokenResponse<UserResponse> userSignIn(SignInRequest request) {
		...
		if (user.getStatus().equals(StatusUser.PENDING)) {
			String message = messageSource.getMessage("error.pendingAccount", null, LocaleContextHolder.getLocale());
		...
		return TokenResponse.<UserResponse>builder().accessToken(accessToken).refreshToken(refreshToken)
	...
	public TokenResponse<EmployerResponse> employerSignIn(SignInRequest request) {
		...
		return TokenResponse.<EmployerResponse>builder().accessToken(accessToken).refreshToken(refreshToken)
				.data(employerResponse).build();
	}
	...
	public TokenResponse<UserResponse> authenticateGoogle(String code) {
		...
		ExchangeTokenGoogleRequest request = ExchangeTokenGoogleRequest.builder().code(code).redirectUri(REDIRECT_URI)
				.clientId(GOOGLE_CLIENT_ID).clientSecret(GOOGLE_CLIENT_SECRET).grantType(GRANT_TYPE).build();
		...
		if (!userRepository.existsByEmail(userInfoResponse.getEmail())) {
			...
			User user = User.builder().email(userInfoResponse.getEmail()).fullName(userInfoResponse.getName())
					.avatarUrl(userInfoResponse.getPicture()).status(StatusUser.ACTIVE).role(role).gender(Gender.MALE)
					.noPassword(true).build();
			User userResponse = userRepository.save(user);
			String createdPasswordToken = jwtService.generateToken(userResponse, TokenType.CREATE_PASSWORD_TOKEN, 1);

			return TokenResponse.<UserResponse>builder().accessToken(null).refreshToken(null).data(null)
					.createPasswordToken(createdPasswordToken).build();
		} else {
			...
			return TokenResponse.<UserResponse>builder().accessToken(accessToken).refreshToken(refreshToken).data(data)
					.createPasswordToken(null).build();
		}
	}
	...
	public TokenResponse<Void> refreshTokenUser(String refreshToken) {
		...
		return TokenResponse.<Void>builder().accessToken(newAccessToken).refreshToken(refreshToken).build();
	}
	...
	public TokenResponse<UserResponse> authenticateLinkedIn(String code) {
		String GRANT_TYPE = "authorization_code";
		ExchangeTokenLinkedInResponse exchangeTokenResponse = linkedInIdentityClient.getToken(GRANT_TYPE, code,
				LINKEDIN_REDIRECT_URI, LINKEDIN_CLIENT_ID, LINKEDIN_CLIENT_SECRET);
		...
		if (!userRepository.existsByEmail(userInfoResponse.getEmail())) {
			...
			UserResponse data = userMapper.toDTO(userResponse);
			data.setRole(userResponse.getRole().getRole());
			return TokenResponse.<UserResponse>builder().accessToken(null).refreshToken(null).data(data)
					.createPasswordToken(createdPasswordToken).build();
		} else {
			User userResponse = userService.findUserByEmail(userInfoResponse.getEmail());
			if (userResponse.getNoPassword()) {
