...
public class JwtServiceImpl implements JwtService {
	...
	public String generateAccessToken(UserDetails userDetails, AccountType accountType) {
		return Jwts.builder().setSubject(userDetails.getUsername()).setId(UUID.randomUUID().toString())
				.setIssuedAt(new Date(System.currentTimeMillis()))
				.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * expiryHour)).setIssuer(provider)
	...
	public String generateRefreshToken(UserDetails userDetails, AccountType accountType) {
		return Jwts.builder().setSubject(userDetails.getUsername()).setId(UUID.randomUUID().toString())
				.setIssuer(provider).setIssuedAt(new Date(System.currentTimeMillis()))
	...
	public String generateToken(UserDetails userDetails, TokenType tokenType, long hour) {
		return Jwts.builder().setSubject(userDetails.getUsername()).setIssuer(provider)
				.setId(UUID.randomUUID().toString()).setIssuedAt(new Date())
				.setExpiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * hour)).signWith(getKey(tokenType))
				.compact();
	}
