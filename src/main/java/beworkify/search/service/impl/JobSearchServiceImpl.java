...
public class JobSearchServiceImpl implements JobSearchService {
  ...
  private JobDocument toDocument(Job job) {
    ...
    JobDocument doc =
        JobDocument.builder()
            ...
            .createdAt(
                job.getCreatedAt() != null ? job.getCreatedAt().atOffset(ZoneOffset.UTC) : null)
            .updatedAt(
                job.getUpdatedAt() != null ? job.getUpdatedAt().atOffset(ZoneOffset.UTC) : null)
    ...
    if (job.getJobTitle() != null) inputs.add(job.getJobTitle());
    if (job.getCompanyName() != null) inputs.add(job.getCompanyName());
  ...
  public PageResponse<List<JobResponse>> searchAdvanced(
    ...
    b.withQuery(
        q ->
            q.bool(
                bool -> {
                  bool.filter(
                      f -> f.term(t -> t.field("status").value(JobStatus.APPROVED.getValue())));
                  ...
                  if (keyword != null && !keyword.isBlank()) {
                    ...
                    bool.should(
                        s ->
                            s.matchPhrasePrefix(
                                mpp ->
                                    mpp.field("requirement").query(normalizedKeyword).boost(0.5f)));
                  }
                                ...
                                t ->
                                    t.field("industries")
                                        .terms(
                                            tv ->
                                                tv.value(
                                                    industryIdList.stream()
                                                        .map(
                                                            id -> FieldValue.of(String.valueOf(id)))
                                                        .toList()))));
                  }
                  ...
                  if (salaryUnit != null && !salaryUnit.isBlank()) {
                    ...
                    if (minSalary != null) {
                      bool.filter(
                          f -> f.range(r -> r.field("maxSalary").gte(JsonData.of(minSalary))));
                    }
                    if (maxSalary != null) {
                      bool.filter(
                          f -> f.range(r -> r.field("minSalary").lte(JsonData.of(maxSalary))));
                    }
                  }
    ...
    SearchHits<JobDocument> hits =
        operations.search(nq, JobDocument.class, IndexCoordinates.of("jobs"));
