...
public class JobSearchServiceImpl implements JobSearchService {
	...
	private String normalizeKeyword(String keyword) {
		if (keyword == null)
			return "";
		return keyword.trim().replaceAll("\\s+", " ").replaceAll("[^\\p{L}\\p{N}\\s]", "").toLowerCase();
	}

	private JobDocument toDocument(Job job) {
		List<Long> industryIds = job.getJobIndustries().stream().map(ji -> ji.getIndustry().getId())
				.collect(Collectors.toList());
		List<Long> provinceIds = job.getJobLocations().stream().map(Location::getProvince).map(p -> p.getId())
				.collect(Collectors.toList());

		JobDocument doc = JobDocument.builder().id(String.valueOf(job.getId())).jobTitle(job.getJobTitle())
				.companyName(job.getCompanyName()).requirement(HtmlImageProcessor.extractText(job.getRequirement()))
				.jobDescription(HtmlImageProcessor.extractText(job.getJobDescription())).industries(industryIds)
				.provinces(provinceIds).expirationDate(job.getExpirationDate())
				...
				.minSalary(job.getMinSalary()).maxSalary(job.getMaxSalary())
				...
				.educationLevel(job.getEducationLevel() != null ? job.getEducationLevel().name() : null).build();
	...
	@Override
	public PageResponse<List<JobResponse>> searchAdvanced(String keyword, List<String> industryIds,
			List<String> provinceIds, List<String> jobLevels, List<String> jobTypes, List<String> experienceLevels,
			List<String> educationLevels, Integer postedWithinDays, Double minSalary, Double maxSalary,
			String salaryUnit, String sort, Pageable pageable) {

		List<Long> industryIdList = industryIds == null
				? List.of()
				: industryIds.stream().filter(StringUtils::hasText).map(Long::valueOf).filter(id -> id > 0).toList();

		List<Long> provinceIdList = provinceIds == null
				? List.of()
				: provinceIds.stream().filter(StringUtils::hasText).map(Long::valueOf).filter(id -> id > 0).toList();
		...
		b.withQuery(q -> q.bool(bool -> {
			...
			if (keyword != null && !keyword.isBlank()) {
				...
				bool.must(m -> m.bool(primary -> {
					primary.should(sq -> sq.bool(jobTitle -> {
						jobTitle.should(exact -> exact
								.term(t -> t.field("jobTitle.keyword").value(originalKeyword).boost(12.0f)));

						jobTitle.should(phrase -> phrase
								.matchPhrase(mp -> mp.field("jobTitle").query(normalizedKeyword).slop(1).boost(9.0f)));

						jobTitle.should(mm -> mm.match(mt -> mt.field("jobTitle").query(normalizedKeyword)
								.operator(Operator.And).fuzziness("1").boost(8.0f)));
					...
					primary.should(sq -> sq.bool(company -> {
						company.should(exact -> exact
								.term(t -> t.field("companyName.keyword").value(originalKeyword).boost(6.0f)));

						company.should(phrase -> phrase.matchPhrase(
								mp -> mp.field("companyName").query(normalizedKeyword).slop(1).boost(5.0f)));

						company.should(mm -> mm.match(mt -> mt.field("companyName").query(normalizedKeyword)
								.operator(Operator.And).fuzziness("1").boost(4.5f)));
				...
				bool.should(s -> s.match(
						mq -> mq.field("jobDescription").query(normalizedKeyword).operator(Operator.And).boost(1.0f)));
				bool.should(s -> s.match(
						mq -> mq.field("requirement").query(normalizedKeyword).operator(Operator.And).boost(1.0f)));

				bool.should(s -> s
						.matchPhrasePrefix(mpp -> mpp.field("jobDescription").query(normalizedKeyword).boost(0.5f)));
				bool.should(
						s -> s.matchPhrasePrefix(mpp -> mpp.field("requirement").query(normalizedKeyword).boost(0.5f)));
			}
			...
			if (postedWithinDays != null && postedWithinDays > 0) {
				...
				bool.filter(f -> f.range(r -> r.field("createdAt").gte(JsonData.of(fromStr))));
			}

			if (!industryIdList.isEmpty()) {
				bool.filter(f -> f.terms(t -> t.field("industries").terms(tv -> tv
						.value(industryIdList.stream().map(id -> FieldValue.of(String.valueOf(id))).toList()))));
			}
			...
			if (jobTypes != null && !jobTypes.isEmpty()) {
				bool.filter(f -> f.terms(
						t -> t.field("jobType").terms(tv -> tv.value(jobTypes.stream().map(FieldValue::of).toList()))));
			}
		...
		List<Long> ids = hits.stream().map(h -> Long.valueOf(h.getContent().getId())).toList();
		...
		return PageResponse.<List<JobResponse>>builder().pageNumber(page.getNumber() + 1).pageSize(page.getSize())
				.totalPages(page.getTotalPages()).numberOfElements(page.getNumberOfElements())
				.items(page.getContent().stream().map(jobMapper::toDTO).toList()).build();
	}
}
